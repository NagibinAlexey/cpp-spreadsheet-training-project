...
namespace ASTImpl {
    ...
    public:
        ...
        virtual double Evaluate(const std::function<double(Position)>& functional_cell) const = 0;
    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(const std::function<double(Position)>& functional_cell) const override {
                double result{};

                switch (type_) {
                case Add:
                    if (lhs_.get()->Evaluate(functional_cell) > std::numeric_limits<double>::max() - rhs_.get()->Evaluate(functional_cell)) {
                        throw FormulaError(FormulaError::Category::Div0);
                    }
                    return lhs_.get()->Evaluate(functional_cell) + rhs_.get()->Evaluate(functional_cell);
                case Subtract:
                    if (-lhs_.get()->Evaluate(functional_cell) > std::numeric_limits<double>::max() - rhs_.get()->Evaluate(functional_cell)) {
                        throw FormulaError(FormulaError::Category::Div0);
                    }
                ...
                case Multiply:
                    ...
                    return lhs_.get()->Evaluate(functional_cell) * rhs_.get()->Evaluate(functional_cell);
                case Divide:
                    result = lhs_.get()->Evaluate(functional_cell) / rhs_.get()->Evaluate(functional_cell);
        ...
        public:
            ...
            double Evaluate(const std::function<double(Position)>& functional_cell) const override {
                if (type_ == UnaryPlus) { return operand_.get()->Evaluate(functional_cell); }
                return -operand_.get()->Evaluate(functional_cell);
            }
...
double FormulaAST::Execute(const std::function<double(Position)>& functional_cell) const {
    return root_expr_->Evaluate(functional_cell);
}
